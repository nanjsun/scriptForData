...
def lu2():
    ...
    age_sheet = book.add_sheet('age')  # 在其中创建一个名为hello的sheet
    ...
    for i in range(int(clos / 2)):
        ...
        age_sheet.write(0, i * 2, table.col_values(i * 2)[0])
        age_sheet.write(0, i * 2 + 1, table.col_values(i * 2 + 1)[0])
        for j in range(rows - 2):
            ...
            hp_index_sheet.write(j + 1, i * 2, stack_id)
            hp_index_sheet.write(j + 1, i * 2 + 1, hp_index)
            age_sheet.write(j + 1, i * 2, stack_id)
            age_sheet.write(j + 1, i * 2 + 1, age)
            print(age)
            print('hpindex: ', hp_index)

    book.save('hpIndexResultWithAge.xls')
